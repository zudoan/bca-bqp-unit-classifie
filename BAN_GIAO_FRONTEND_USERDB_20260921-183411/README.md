...
Database tài khoản được khởi tạo tự động tại `storage/users.db`. Có thể đổi vị
trí hoặc chuyển sang một database SQLAlchemy khác bằng biến môi trường
`USER_DATABASE_URL`. Khi chạy bằng Docker Compose, file SQLite nằm trong volume
`bca-bqp-user-data` nên dữ liệu tài khoản vẫn được giữ lại sau khi build lại
container.

Để tạo hoặc bảo đảm tài khoản quản trị đầu tiên, khai báo trong file `.env`
...
Đổi `AUTH_COOKIE_SECURE=true` khi triển khai frontend/backend qua HTTPS. API xác
thực nằm tại `/api/v1/auth/*`; API quản trị người dùng nằm tại
`/api/v1/admin/users` và chỉ chấp nhận tài khoản có role `admin`. Hướng dẫn kiểm
tra chi tiết nằm trong `BACKEND_USER_DATABASE_GUIDE.md`.
